  ...
  <!--formaulario editar noticia -->
    ...
    <div class="bg-indigo-50 max-h-fit md:px-20 py-6">
      <div class=" bg-white rounded-md px-6 py-3 max-w-2xl mx-auto">
        <h1 class="text-center text-2xl font-bold text-gray-500 mb-5">Editar Titular</h1>
        <div class="space-y-2">
            ...
            <textarea id="headline" name="headline" cols="30" rows="3" placeholder="escriba aquí.." class="w-full   p-4 text-gray-600 bg-indigo-50 outline-none rounded-md">{{$headline->headline}}</textarea>
          </div>
          <div>
              ...
              <div class="mb-3 xl:w-96">
                <select name="owner" class="form-select appearance-none block w-4/5 px-3 py-1.5 text-gray-700 bg-white bg-clip-padding bg-no-repeat border  border-gray-300  rounded-md transition
                  ease-in-out ml-3 focus:text-gray-700 focus:bg-white focus:border-gray-600 focus:outline-none" >
                    ...
                    @if ($i->id == $headline->owner)
                    <option selected value="{{$i->id}}">{{$i->web_site_name}}</option>
                    @endif
                    <option value="{{$i->id}}">{{$i->web_site_name}}</option>
                    @endforeach
